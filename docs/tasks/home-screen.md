...
- Text: from `content.app.name` (e.g. "Staya" or "STAYA" — see `src/frontend/core/content.ts`).
...
- Use **assets getter:** `src/frontend/core/assets.ts` → `assets.icons.hotel`, `.flight`, `.holidays`.
...
- **Strings and list data:** All in `src/frontend/core/content.ts` (e.g. `content.app.name`, `content.nav.*`, `content.hero.*`). No hardcoded copy in components.
- **Icons (nav tabs):** Paths from `src/frontend/core/assets.ts` → `assets.icons.hotel`, `.flight`, `.holidays` (pointing to `public/icons/*.png`).
...
| Copy / strings       | `src/frontend/core/content.ts` |
| Icon/image paths     | `src/frontend/core/assets.ts` |
