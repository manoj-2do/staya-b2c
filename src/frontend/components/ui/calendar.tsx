  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        day: "h-8 w-8 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-range-start)]:rounded-l-[9999px] first:[&:has([aria-selected])]:rounded-l-[9999px] last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
        day_button: cn(
          buttonVariants({ variant: "ghost", size: "icon" }),
          "h-8 w-8 p-0 font-normal aria-selected:opacity-100 rounded-md"
        ),
        range_start: "day-range-start rounded-l-[9999px] rounded-r-none bg-primary text-white",
        range_end: "day-range-end rounded-r-md rounded-l-none bg-primary text-white",
        selected:
